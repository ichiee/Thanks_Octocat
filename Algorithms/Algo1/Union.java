...
Each entry array contains - referenceto the parent tree.

From this data structure you can go to root - 

Once you know the root you can impliment.
...
So class QuickUnionUF have an constructtor 

then finding root function
check if the 2 entry p & q has the same root by using another method  to return boolean
...
public class QuickUnionUF {
    private int[] parent;  // parent[i] = parent of i // this is the same as id 
    private int count;     // number of components
     ...
     * @throws IllegalArgumentException if {@code n < 0}
     */
    public QuickUnionUF(int n) {.           // this is constructing parent array 
        parent = new int[n];
        count = n;
        for (int i = 0; i < n; i++) {
            parent[i] = i;
    ...
    // validate that p is a valid index
    private void validate(int p) {
        int n = parent.length;     // this would be array of the parent
        if (p < 0 || p >= n) {
            throw new IllegalArgumentException("index " + p + " is not between 0 and " + (n-1));
        }
    }
...
public class WeightedQuickUnionUF {
     ...
     * @throws IllegalArgumentException unless {@code 0 <= p < n}
     */
    public int find(int p) {.   // this is find root the same as quick union
        validate(p);
        while (p != parent[p])
            p = parent[p];
        return p;
    ...
    /**
     * Returns true if the the two sites are in the same component.   // if the roots are the same
     *
     * @param  p the integer representing one site
     * @param  q the integer representing the other site
     * @return {@code true} if the two sites {@code p} and {@code q} are in the same component;
     ...
     */
    public void union(int p, int q) {
        int rootP = find(p);                // this is by finding root of p - that should the index == value of parent []
        int rootQ = find(q);                
        if (rootP == rootQ) return;

        // make smaller root point to larger one
        if (size[rootP] < size[rootQ]) {            // if index q of size has begger value
            parent[rootP] = rootQ;                  // indes p of parent will take value / index of q
            size[rootQ] += size[rootP];             // bigger size - size [index q] will have a value of size [p]
        }
        else {
            parent[rootQ] = rootP;                  // otherwise do oppositeway around. either way a bigger size will be added more
